...
import { categories, districts, officers, transitions, type Complaint } from '../../../lib/domain';
import { z } from 'zod';
import { allowed, authorized, actor as currentActor } from '../../../lib/access';
...
export async function GET(req: Request) {
    ...
    try {
        ...
        return Response.json({ complaints }, { headers: { 'Cache-Control': 'no-store' } });
    }
    catch (e) {
        console.error(e);
...
export async function POST(req: Request) {
    ...
    try {
        ...
        const db = complaintStore();
        if (body && typeof body === 'object' && 'action' in body && body.action === 'curate-demo-15') {
            return Response.json({ archived: await db.curate() });
        }
...
export async function PATCH(req: Request) {
    ...
    try {
        ...
        const body = parsed.data, db = complaintStore();
        const row = await db.find(body.id);
        if (!row)
        ...
        const c: Complaint = structuredClone(row), oldOfficer = c.officer;
        ...
        if (resolving) {
            ...
            if (file.size > MAX_PHOTO_BYTES)
                return fail('Photo must be 4 MB or smaller. / फोटो 4 MB या कम होनी चाहिए।', 413);
        ...
        const saved = await db.save(c, row.version);
        if (!saved) {
            if (uploadedKey)

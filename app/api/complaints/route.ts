...
const coordinatesTogether=(v:{latitude:number|null;longitude:number|null})=>(v.latitude===null)===(v.longitude===null);
const newComplaint = z.object({title:z.string().trim().min(1).max(500),category:z.enum(categories),district:z.string().refine(x=>districts.includes(x)),address:z.string().trim().min(1).max(500),citizen:z.string().trim().min(1).max(500),priority:z.enum(['Critical','High','Normal']),...locationFields}).refine(coordinatesTogether);
const update = z.object({id:z.string(),version:z.coerce.number().int(),status:z.string(),officer:z.string().refine(x=>['',...officers].includes(x)),note:z.string().trim().min(1).max(2000),address:z.string().trim().min(1).max(500).optional(),district:z.string().refine(x=>districts.includes(x)).optional(),...locationFields}).refine(coordinatesTogether);
...
export async function PATCH(req:Request){
 ...
 try{
  ...
  const parsed=update.safeParse(raw);if(!parsed.success)return fail('Check the location and enter an activity note. / स्थान जाँचें और गतिविधि नोट भरें।');
  ...
  if(raw&&typeof raw==='object'&&!('latitude' in raw)&&!('longitude' in raw)){body.latitude=c.latitude??null;body.longitude=c.longitude??null;}
  ...
  if(resolving){
   if(!file||typeof file==='string'||!file.size)return fail('Add a photo of the completed repair before resolving. / निस्तारण से पहले मरम्मत की फोटो जोड़ें।');
   if(file.size>MAX_PHOTO_BYTES)return fail('Photo must be 5 MB or smaller. / फोटो 5 MB या कम होनी चाहिए।',413);
   const bytes=new Uint8Array(await file.arrayBuffer()),mime=imageType(bytes);
   if(!mime||mime!==file.type)return fail('Upload a JPEG, PNG or WebP photo. / JPEG, PNG या WebP फोटो चुनें।');
  ...
  }else if(file&&typeof file!=='string'&&file.size)return fail('Select Resolved to submit a repair photo. / मरम्मत की फोटो जमा करने के लिए निस्तारित चुनें।');
  const locationChanged=(body.address!==undefined&&body.address!==c.address)||(body.district!==undefined&&body.district!==c.district)||body.latitude!==(c.latitude??null)||body.longitude!==(c.longitude??null);
  if(body.address!==undefined)c.address=body.address;
  if(body.district!==undefined)c.district=body.district;
  c.latitude=body.latitude;c.longitude=body.longitude;
  ...
  if(resolving){c.resolvedAt=now;c.resolution=body.note;c.resolutionPhoto=photo;}
  if(body.status==='Reopened'&&previousStatus!=='Reopened'){c.resolvedAt=null;c.resolution='';c.resolutionPhoto=null;}
  const actor=req.headers.get('oai-authenticated-user-email')||'Local department head';
  c.history.push({at:now,action:body.status,note:body.note+(body.officer!==oldOfficer?` Officer: ${body.officer}.`:'')+(locationChanged?` Location: ${c.district}, ${c.address}${c.latitude!==null?` (${c.latitude}, ${c.longitude})`:''}.`:'')+` (${actor})`,...(photo?{photo}: {})});
  ...
  if(!result.meta.changes){if(uploadedKey)await bucket().delete(uploadedKey);return fail('This complaint changed. Reload before saving. / शिकायत बदल गई है। पुनः लोड करें।',409);}
  uploadedKey=null;
  return Response.json(c);
 }catch(e){console.error(e);if((e as Error).message==='UPLOAD_TOO_LARGE')return fail('Photo must be 5 MB or smaller. / फोटो 5 MB या कम होनी चाहिए।',413);return fail('Unable to save. Your photo and note can be retried. / सहेजा नहीं गया। फोटो और नोट के साथ पुनः प्रयास करें।',503);}
}

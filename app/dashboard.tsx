...
import { ArrowDownToLine, ArrowRight, ChevronRight, FileText, LayoutDashboard, MapPin, Menu, Plus, RefreshCw, Search, Users, X } from 'lucide-react';
import { districts, officers, statuses, type Complaint } from '../lib/domain';
...
const navigation = [{ name: 'Overview', icon: LayoutDashboard }, { name: 'Complaints', icon: FileText }, { name: 'Officers', icon: Users }] as const;
...
export default function Dashboard() {
    ...
    const [collapsed, setCollapsed] = useState(false), [smallScreen, setSmallScreen] = useState(false);
    useEffect(() => {setCollapsed(localStorage.getItem('up-sidebar-hidden') === 'true'); const media=window.matchMedia('(max-width:640px)');const update=()=>setSmallScreen(media.matches);update();media.addEventListener('change',update);return()=>media.removeEventListener('change',update);}, []);
    ...
    const drill = (risk: Risk) => navigate('Complaints', { risk });
    const riskOptions: [
        Risk,
        string,
        number
 ...
 <aside id="navigation-sidebar" className={'sidebar ' + (mobile ? 'expanded' : '')} aria-label={t('Operations')}><a className="brand" href="#main" onClick={() => navigate('Overview')}><span>UP<span className="brand-accent">.one</span></span></a><div className="department"><span>{t('Government of Uttar Pradesh')}</span><strong>{t('Electrical Department')}</strong></div><nav>{navigation.map(({ name, icon: Icon }) => <button key={name} aria-current={view === name ? 'page' : undefined} className={view === name ? 'active' : ''} onClick={() => navigate(name)}><Icon size={18}/><span>{t(name)}</span>{name === 'Complaints' && <b>{items.length}</b>}</button>)}</nav><div className="sidebar-bottom"><strong>{t('Department Head')}</strong></div></aside>
 <div className="workspace"><header className="topbar"><div className="breadcrumb"><button ref={menuRef} className="icon-button sidebar-toggle" aria-label={t(smallScreen ? (mobile ? 'Close navigation' : 'Open navigation') : (collapsed ? 'Show sidebar' : 'Hide sidebar'))} aria-controls="navigation-sidebar" aria-expanded={smallScreen ? mobile : !collapsed} onClick={() => {if(smallScreen)setMobile(!mobile);else {setCollapsed(!collapsed);localStorage.setItem('up-sidebar-hidden',String(!collapsed));}}}><Menu size={22}/></button><span>{t('Electrical Department')}</span><ChevronRight size={14}/><strong>{t(view)}</strong></div><div className="topbar-actions"><div className="language" role="group" aria-label={t('Language')}><button lang="en" aria-pressed={language === 'en'} onClick={() => setLanguage('en')}>English</button><button lang="hi" aria-pressed={language === 'hi'} onClick={() => setLanguage('hi')}>हिन्दी</button></div><HeaderActions items={items} now={now} t={t} onOpen={setSelected}/></div></header>
 ...
 <section className="attention panel"><div className="panel-heading"><h2>{t('Needs attention')}</h2><p>{t('Counts may overlap')}</p></div><div className="attention-actions">{([['critical', 'Critical', m.critical], ['near', 'Due within 48 hours', m.near], ['unassigned', 'Unassigned', m.unassigned], ['escalated', 'Active escalations', m.escalated]] as [
                ...
                string,
                number
            ][]).map(([risk, label, n]) => <button key={risk} className={"attention-" + risk} onClick={() => drill(risk)}><strong className={risk === 'critical' && n ? 'critical-text' : ''}>{n}</strong><span>{t(label)}</span><ChevronRight size={17}/></button>)}</div></section>

 <div className="charts"><Trend items={base} t={t} language={language} now={now}/><RecurringPie items={base} t={t} onSelect={recurrence => navigate('Complaints', { recurrence })}/></div><div className="overview-geography"><section className="panel"><div className="panel-heading"><div><h2>{t('District concentration')}</h2><p>{t('Complaints by district')}</p></div></div>{districts.map(d => ({ d, count: base.filter(c => c.district === d).length, late: base.filter(c => c.district === d && matchesRisk(c, 'overdue', now)).length })).filter(x => x.count).sort((a, b) => b.count - a.count).map(({ d, count, late }) => <button className="district-row" key={d} onClick={() => navigate('Complaints', { district: d })}><span><strong>{t(d)}</strong><small>{late} {t('overdue')}</small></span><span className="district-bar"><i style={{ width: `${count / Math.max(1, m.total) * 100}%` }}/></span><b>{count}</b><ChevronRight size={16}/></button>)}{!base.length && <div className="empty">{t('No matching districts')}</div>}</section></div></>}
  {view === 'Complaints' && <>
 <section className="panel"><div className="list-filters"><label>{t('Attention filter')}<select value={filters.risk} onChange={e => patch({ risk: e.target.value as Risk })}>{riskOptions.map(([risk,label]) => <option key={risk} value={risk}>{t(label)}</option>)}<option value="unassigned">{t('Unassigned')}</option></select></label><label>{t('Status')}<select value={filters.status} onChange={e => patch({ status: e.target.value })}><option value="">{t('All statuses')}</option>{statuses.map(s => <option key={s} value={s}>{t(s)}</option>)}</select></label><label>{t('Priority')}<select value={filters.priority} onChange={e => patch({ priority: e.target.value })}><option value="">{t('All priorities')}</option>{['Critical', 'High', 'Normal'].map(s => <option key={s} value={s}>{t(s)}</option>)}</select></label><label>{t('Assigned officer')}<select value={filters.officer} onChange={e => patch({ officer: e.target.value })}><option value="">{t('All officers')}</option>{officers.map(s => <option key={s} value={s}>{t(s)}</option>)}</select></label><label>{t('Sort by')}<select value={filters.sort} onChange={e => patch({ sort: e.target.value as Sort })}><option value="priority">{t('Priority and deadline')}</option><option value="oldest">{t('Oldest first')}</option><option value="newest">{t('Newest first')}</option></select></label></div>

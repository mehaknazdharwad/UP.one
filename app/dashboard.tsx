...
export default function Dashboard() {
 ...
 <section className="panel queue"><div className="panel-heading"><div><h2>{t('Priority queue')}</h2><p>{t('Critical first, then earliest deadline')}</p></div><button className="text-button" onClick={() => navigate('Complaints')}>{t('View all complaints')}<ArrowRight size={16}/></button></div><ComplaintTable rows={selectComplaints(base.filter(c => !done(c)), { ...emptyFilters }, now).slice(0, 5)} t={t} now={now} onOpen={setSelected}/></section>

...
export function ComplaintDialog({ c, t, date, sla, now, saving, close, save, reload }: {
 ...
 <form className="update-form" onSubmit={submit} onChange={() => setDirty(true)}><h3>{t('Update complaint')}</h3><fieldset disabled={saving} className="plain-fieldset"><div className="form-grid"><label>{t('Assigned officer')}<select aria-label={t('Assigned officer')} value={officer} onChange={e => { setOfficer(e.target.value); if (c.status === 'New' && e.target.value && status === 'New')
        ...
        setStatus('New'); }} required={status !== 'New'}>{['', ...officers].map(o => <option key={o} value={o}>{o || t('Unassigned')}</option>)}</select></label><label>{t('Status')}<select aria-label={t('Status')} value={status} onChange={e => { setStatus(e.target.value); setError(''); if (e.target.value === 'New')
 ...
 {resolving && <div className="photo-upload"><label htmlFor="resolution-photo"><span><Upload size={16}/>{t('Resolution photo')} · {t('Required')}</span><small>{t('Upload a photo showing the completed repair. JPEG, PNG or WebP · up to 5 MB.')}</small></label><input id="resolution-photo" name="photo" type="file" accept="image/jpeg,image/png,image/webp" onChange={e => { const f = e.target.files?.[0] || null; setPhoto(null); setError(''); if (f && (!['image/jpeg', 'image/png', 'image/webp'].includes(f.type) || f.size > 5 * 1024 * 1024 || !f.size)) {
 ...
 <section className="detail-section"><h3>{t('Escalation history')} <span className="count">{escalations.length}</span></h3>{escalations.length ? history(escalations) : <p>{t('No escalation recorded')}</p>}</section><details className="history-details" open><summary>{t('Activity & history')} <span className="count">{c.history.length}</span></summary>{history(c.history)}</details><p className="section-note">{t('SLA clock starts when reported; reopening keeps the original deadline.')}</p></div></Dialog>;
}

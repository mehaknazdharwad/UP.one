...
export function seed():Complaint[]{
 ...
 return Array.from({length:84},(_,i)=>{const age=3+(i*7%26)+i%3/3; const createdAt=new Date(Date.now()-age*86400000).toISOString(); const status=i<6?['Escalated','In progress','New','Assigned','Reopened','In progress'][i]:statuses[(i*3+2)%7];const resolvedAt=['Resolved','Closed'].includes(status)?new Date(new Date(createdAt).getTime()+Math.min(age,2+i%8)*86400000).toISOString():null;return {id:`UP-EL-${20260001+i}`,title:titles[i%6],category:categories[i%6],district:districts[(i*3)%8],address:`Ward ${12+i%20}, ${['Gomti Nagar','Civil Lines','Station Road','Indira Nagar'][i%4]}`,citizen:['Aarav Singh','Pooja Gupta','Mohd. Ali','Ritu Verma'][i%4],priority:i%9===0?'Critical':i%3===0?'High':'Normal',status,officer:status==='New'?'':officers[i%6],createdAt,updatedAt:createdAt,resolvedAt,resolution:resolvedAt?'Fault repaired, supply checked and service restored.':'',history:[{at:createdAt,action:'New',note:'Complaint received through citizen portal.'},...(status==='New'?[]:[{at:createdAt,action:status,note:'Routed to the Electrical Department.'}])],version:1};});
}
